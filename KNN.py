...
class KNNClassifier:
    ...
    def fit(self,X_train,y_train):
        '''根据训练数据集'X_train和y_train训练KNN分类器'''
    ...
    def predict(self,X_predict):
        '''给定待遇测数据集X_train和y_train，返回表示X_predict的结果向量'''
        assert self._X_train is not None and self._X_train is not None,"must fit before predict"
        assert X_predict.shape[1] == self._X_train.shape[1], "the feature number of x must equal to X_train"
